...
time_t F4RTC::getTime()
{
	...
	tStruct.tm_year = (((reg32 & CYGHWR_HAL_STM32_RTC_DR_YT) >> CYGHWR_HAL_STM32_RTC_DR_YT_SHIFT) * 10) + 100 + ((reg32 & CYGHWR_HAL_STM32_RTC_DR_YU) >> CYGHWR_HAL_STM32_RTC_DR_YU_SHIFT);
...
void F4RTC::syncEcosTime(cTerm & term, int argc,char * argv[])
{
	time_t t = time(0);
	setTime(&t);
	if(syncTime(t))
